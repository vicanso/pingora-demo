...
impl Server {
    pub fn new(addr: &str, upstream: &str) -> Self {
        Self {
            addr: addr.to_string(),
...
impl ProxyHttp for Server {
    ...
    async fn upstream_peer(
        ...
        // 获取对应的upstream
        let up = get_upstream(&self.upstream).ok_or(new_internal_error(
            500,
...
// RUST_LOG=INFO cargo run --example 完善upstream
fn main() {
    env_logger::init();
    // 初始化pingora实例
    ...
    let server = Server::new("127.0.0.1:6118", charts_upstream);
    // 添加http服务至当前实例中
